...
last_updated: "2026-06-23T06:42:49Z"
...
slides:
  ...
  - { id: "17",  beat: "Axis 2 — \"Make is all you need\"",          phase: "5", act: "2",    layout: "purdue-content",      code_aesthetic: "definition",  must_not_skip: false, compressible: false, breath: false, status: "complete", assets: [], note: "COMPLETE — the FLOOR of the orchestration axis and the FIRST consumer of the .code-definition device (THEME §10), fleshed out from its Phase-1 stub: a LIGHT authored-source card with an editor filename TAB ('Makefile') + syntax tints drawn from the deck's layer vocabulary (slate target = orchestration #2f567f, green prerequisite = data, gold automatic-vars $< $@) — deliberately the INVERSE of the dark .code-terminal window (Slides 13–16) so 'authored vs. run' reads instantly. NEW Axis-2 .axis-tag: slate --c + a 2-DOT ratchet (●○ 'GNU Make'), the parallel to the executor climb's 4-dot ratchet (Axis 2 has only two rungs, Make → Nextflow). Snippet is the OUTLINE §4 Makefile (blank separator line dropped + .cd-body sized a shade under the terminal so the long patsubst line stays on one row and the 17b keeper clears the bottom rule; $</$> escaped $&lt;/&gt; for MDC; $@/$(...)/%/* survive inline). h1 IS the provocation 'Make is all you need'; the planted 'OUT OF THE BOX' is the on-slide subhead emphasis (+ notes). Build + export green; eyeball-verified page 17 (subhead trimmed to one line + spacing tightened in review so the keeper cleared the page-num)." }
  - { id: "17b", beat: "Axis 2 — how far Make reaches (reverse)",  phase: "5", act: "2",    layout: "purdue-content",      code_aesthetic: "definition",  must_not_skip: false, high_value: true, compressible: false, breath: false, status: "complete", assets: [], note: "COMPLETE — slide-REVERSE of 17, NOT a new physical slide (deck stays 24 physical slides). Realized on the SAME Slide 17 page: a single `v-click` (Slide 2 precedent; Slidev exports the final composited frame, so the PDF shows the reframe) reveals the .mk-reframe layer over the SAME Makefile — 'same code, new lens'. The four shims Make can absorb are a brisk muted chip row (.mk-shims: containers·$(RUN) · cluster·srun · resume·atomic mv · modules·child-Makefiles) = scenery, wave don't teach. The KEEPER is the .mk-wires gold felt-question — recurring line 'who's holding the wires?' fires VERBATIM on-slide ('It's who's holding the wires — you, or a framework you now have to feed?'); this is its PRIMARY firing (reprised verbally on Slides 16 + 19). Control, not capability — a felt question, not a 2nd formal axis. REVIEW NOTE for Geoffrey: the 17→17b reveal is a forward v-click rather than a literal flip-back, and the shims are a chip row (not margin call-outs on the recipe line) — say if you'd prefer static annotations or true reverse-navigation. Build + export green." }
  - { id: "18",  beat: "Axis 2 — Nextflow as justified endpoint",  phase: "5", act: "2",    layout: "purdue-content",      code_aesthetic: "definition",  must_not_skip: false, high_value: true, compressible: false, breath: false, status: "complete", assets: [], note: "COMPLETE — the TOP of the orchestration axis (ratchet ●●) and a must-not-skip beat. Like Slide 16 at the top of Axis 1, it holds BOTH scenery (the dense main.nf snippet + a 5-feature strip) AND a must-land IDEA (human scalability = the packaging-index move), the IDEA carrying the visual weight via the gold .nf-keep keeper so the snippet never eats its air. Reuses the .code-definition device from Slide 17 with a NEW .is-dense modifier (smaller font + tighter leading) so the 7-line snippet reads as 'a lot of machinery' and stays in the vertical budget (the Slide 17 lesson). axis-tag → slate Axis-2 ratchet ●● ('Nextflow'), completing the orchestration climb (Make ●○ → Nextflow ●●). Groovy reuses the deck's definition tints: process/workflow/directives = mauve .cd-kw, process name = slate .cd-tgt (parallel to a Make target), strings = green .cd-str, ${...} interpolation = gold .cd-var (parallel to the Makefile's gold $< $@). main.nf is the OUTLINE §4 snippet verbatim (only the script-string redirect > escaped &gt;; {}/${}/*/quotes survive MDC inline). 5-feature strip (.nf-feat, slate sibling of Slide 16's teal .hs-feat): containers · dynamic DAG · -resume · executor portability · nf-core. h1 IS the recurring 'earn its keep' reprise + names the human-scale must-land; .nf-keep is the packaging-index keeper (gold-rule statement, sibling of Slide 17's .mk-wires); .nf-foot fires the recurring 'the map outlasts the tools' on-slide in one muted line (own-the-bias tool list in notes). Honest-cost / relocates-control (the 17b who-holds-the-wires callback) / climb-back-without-guilt all in notes. Build + export green; eyeball-verified page 18 (keeper converted box→rule + snippet/margins tightened in review so the map-outlasts foot cleared the bottom and the page-num)." }
  - { id: "19",  beat: "The decision ladder + checklist",          phase: "5", act: "2",    layout: "two-axis-ladder",     code_aesthetic: "none",        must_not_skip: true,  compressible: false, breath: true,  status: "complete", assets: [], note: "COMPLETE — THE PHOTOGRAPH-SLIDE; the single portable artifact + the payoff of the whole talk. Completes the two-axis-ladder layout via the .is-full form (Slide 12 preview's absolute .ta-stage replaced by .taf-grid): LEFT = the full ladder, both axes reusing the Slide 12 rung grammar compacted so Axis-1's five rungs (bash loop → xargs -P → Slurm array → HyperShell → HTCondor) fit ONE line so the photograph matches what the room saw on Slide 12. HTCondor is the greyed/dashed .is-ghost 'heavyweight above' rung, bound to its leading arrow via .ta-beyond (white-space:nowrap) so it can never orphan to a new line as a phantom first rung. Under each track, the .taf-grad colour wash is the load-bearing VISUAL of the slide: left = craft-gold 'you', right = axis-tint 'framework', with muted 'you hold the wires / a framework holds them' end-labels = the felt question 'who's holding the wires?' rendered as a picture, NOT a 2nd formal dimension (OUTLINE reframe). RIGHT = the .taf-chk checklist grouped under the two axis headings (axis-tinted ✓ bullets); the Axis-1 list carries the Slide 16 pressure-release-valve callback ('…or you'd DDoS the shared scheduler?'). The single boxed takeaway (.taf-take, gold left-rule keeper, sibling of Slide 18's .nf-keep) fires 'earn its keep' on-slide AND lands BOTH directions (No → you're done; Yes → climb without guilt) so it isn't purely deflationary. Recurring lines fire: 'Which axis are you on?' (subhead + notes), 'Does it earn its keep?' (takeaway 'earn its keep' on-slide + verbatim notes), 'Each rung adds one capability' (rung tracks carry it visually + notes), 'Who's holding the wires?' (gradient end-labels + said once verbally, notes), 'The map outlasts the tools' (HTCondor greyed past HyperShell + notes). [BREATH] / must-not-skip — kept airy; slowest beat in Act 2. HTML/CSS only — no inline SVG (MDC hazard); &nbsp; keeps 'xargs -P' intact. PDF stays 19 pages (24 physical slides total; 17b is a v-click on the 17 page, not a new page). Build + export green; eyeball-verified at page 19 (track compacted in review so all five rungs sit on one line)." }
...
layouts:
  ...
  - { name: "two-axis-ladder",     kind: "custom",    first_slide: "12", status: "complete", note: "COMPLETE — both forms built. PREVIEW form (Slide 12, Phase 4): two clearly-separated horizontal axis tracks (teal executor / slate orchestration), `→`-joined rung chips, per-axis question, + faint greyed unclimbed-tool field showing the chosen subset; .ta-foot renders the map-outlasts-the-tools payoff. FULL form (Slide 19, Phase 5): the `.is-full` modifier replaces the preview's absolute .ta-stage with a .taf-grid LEFT-ladder / RIGHT-checklist split — the full ladder (rungs compacted to one line; HTCondor the greyed .is-ghost 'heavyweight above', arrow-bound via .ta-beyond), the load-bearing .taf-grad you-own-it → framework-owns-it colour wash under each track (with 'who's holding the wires?' end-labels), the axis-grouped .taf-chk checklist (axis-tinted ✓ bullets + Slide 16 DDoS callback), and the gold .taf-take both-directions keeper. Content-family chrome (gold rules, RCAC mark, page-num) on paper throughout. HTML/CSS only — no inline SVG. The Slide 19 gradient is reused by the Phase-6 ownership-ladder layout." }
...
* [x] **Slide 17 — "Make is all you need"** (definition aesthetic). **DoD:**
      small Makefile in the *definition* treatment (distinct from terminal);
      the "out of the box" provocation planted. *Done — first consumer of the
      `.code-definition` device: a LIGHT authored-source card (editor filename
      tab + slate-target / green-prerequisite / gold-automatic-var syntax tints)
      — the deliberate inverse of the dark `.code-terminal` window, so
      "authored vs. run" reads instantly. NEW Axis-2 `.axis-tag` (slate `--c`,
      2-dot ratchet ●○ "GNU Make"). h1 is the provocation; "out of the box" is
      the planted subhead emphasis. Build + export green; page 17 verified.*
* [x] **Slide 17b — how far Make reaches** (**slide-REVERSE of 17 — NOT a new
      physical slide**). **must-not-skip.** **DoD:** reuse the Slide 17
      Makefile verbatim, talk over it (optional revealed margin annotations);
      recurring line *"who's holding the wires?"* as a felt question. *Done —
      realized on the SAME Slide 17 page via a single `v-click` (Slide 2
      precedent) that reveals the `.mk-reframe` layer over the same Makefile:
      the four shims as a brisk chip row + the gold `.mk-wires` keeper. The
      recurring line fires VERBATIM on-slide (its PRIMARY firing). No 25th
      physical slide — deck stays 24. Build + export green.*
* [x] **Slide 18 — Nextflow as justified endpoint** (definition). **must-not-
      skip.** **DoD:** minimal Nextflow process/channel + 5-feature callout;
      the **human-scalability / packaging-index** argument given room;
      recurring line *"the map outlasts the tools"*; own the bias. *Done —
      reuses the `.code-definition` device (new `.is-dense` modifier for the
      7-line `main.nf`), slate Axis-2 `.axis-tag` ●● completing the climb. The
      must-land packaging-index / human-scalability idea is the gold `.nf-keep`
      keeper (gets room); the 5 features are the slate `.nf-feat` strip; h1
      fires the "earn its keep" reprise; `.nf-foot` fires "the map outlasts the
      tools" on-slide + owns the bias (tool list in notes). Build + export
      green; page 18 verified.*
* [x] **Slide 19 — decision ladder + checklist** (`two-axis-ladder`, full).
      **must-not-skip · [BREATH].** **DoD:** both axes + rungs in one picture
      ...
      (stop when no; climb without guilt when yes). *Done — completes the
      `two-axis-ladder` layout with the `.is-full` form (the Slide 12 preview's
      absolute `.ta-stage` replaced by `.taf-grid`): LEFT = the full ladder,
      both axes reusing the Slide 12 rung grammar (compacted so Axis-1's five
      rungs fit one line so the photograph matches what the room saw), HTCondor
      the greyed `.is-ghost` rung past HyperShell (bound to its arrow via
      `.ta-beyond` so it can never orphan as a phantom first rung). Under each
      track, the `.taf-grad` left→right colour wash (craft-gold "you" →
      axis-tint "framework") with "you hold the wires / a framework holds them"
      end-labels = the felt question as a VISUAL. RIGHT = the axis-grouped
      `.taf-chk` checklist (axis-tinted ✓ bullets); the Axis-1 list carries the
      Slide 16 DDoS callback. The gold-left-rule `.taf-take` keeper fires "earn
      its keep" on-slide AND lands both directions. Recurring lines all fire
      ("which axis are you on?" subhead; "earn its keep" takeaway; "does it earn
      its keep?" / "each rung adds one capability" / "who's holding the wires?"
      verbatim in notes). HTML/CSS only — no inline SVG. Build + export green;
      eyeball-verified at page 19 (track compacted in review so all five rungs
      fit one line; airy [BREATH] spacing kept).*
* [x] Commit(s): `WIP: Slide N — <beat>`
...
*Last updated: 2026-06-23T06:10:44Z*
